...
re-stitch has replaced is *stale*: it corrects nothing, reports zero count
with a `SCRATCHES_STALE` warning, and needs re-detecting. `roll info` gains
a per-negative `scratches` **summary**
