...
struct CLISessionTests {
    ...
    func successfulRun() async throws {
        try await TestSupport.withTemporaryDirectory { directory in
            let executable = try TestSupport.writeTestExecutable(
                #"""
                printf '%s\n' '{"protocol_version":4,"event":"started","command":"probe"}'
                printf '%s\n' 'reading /tmp/input' 1>&2
                printf '%s\n' '{"protocol_version":4,"event":"probe_result","catalogue":["_DSC4638.NEF"],"warnings":[],"groups":[]}'
                printf '%s\n' '{"protocol_version":4,"event":"finished","status":"complete","exit_status":0}'
                exit 0
                """#,
                in: directory
            )
    ...
    func structuredFailure() async throws {
        try await TestSupport.withTemporaryDirectory { directory in
            let executable = try TestSupport.writeTestExecutable(
                #"""
                printf '%s\n' '{"protocol_version":4,"event":"started","command":"convert","run_id":"r1"}'
                printf '%s\n' '{"protocol_version":4,"event":"error","run_id":"r1","code":"OUTPUT_CONFLICT","message":"3 outputs already exist"}'
                printf '%s\n' '{"protocol_version":4,"event":"finished","run_id":"r1","status":"failed","exit_status":1}'
                exit 1
                """#,
                in: directory
            )
    ...
    func cooperativeCancellationExiting143() async throws {
        try await TestSupport.withTemporaryDirectory { directory in
            let executable = try TestSupport.writeTestExecutable(
                #"""
                on_term() {
                  printf '%s\n' '{"protocol_version":4,"event":"error","run_id":"r1","code":"CANCELLED","message":"cancelled by the user"}'
                  printf '%s\n' '{"protocol_version":4,"event":"finished","run_id":"r1","status":"cancelled","exit_status":143}'
                  exit 143
                }
                trap on_term TERM
                printf '%s\n' '{"protocol_version":4,"event":"started","command":"convert","run_id":"r1"}'
    ...
    func cancellationHonouredWithinTheGracePeriodIsNotForced() async throws {
        try await TestSupport.withTemporaryDirectory { directory in
            let executable = try TestSupport.writeTestExecutable(
                #"""
                on_term() {
                  printf '%s\n' '{"protocol_version":4,"event":"finished","status":"cancelled","exit_status":143}'
                  exit 143
                }
                trap on_term TERM
                printf '%s\n' '{"protocol_version":4,"event":"started","command":"convert"}'
                i=0
                while [ $i -lt 1200 ]; do sleep 0.05; i=$((i + 1)); done
                """#,
                in: directory
    ...
    func largeOutputOnBothStreamsDoesNotDeadlock() async throws {
        try await TestSupport.withTemporaryDirectory { directory in
            ...
            let executable = try TestSupport.writeTestExecutable(
                ...
                i=0
                while [ $i -lt 800 ]; do
                  printf '{"protocol_version":4,"event":"warning","code":"FILENAME_SORT_USED","message":"%s"}\n' "$PAD"
                  printf 'log %s\n' "$PAD" 1>&2
                  i=$((i + 1))
                done
                printf '%s\n' '{"protocol_version":4,"event":"finished","status":"complete","exit_status":0}'
    ...
    func splitJSONFromARealPipeIsReassembled() async throws {
        try await TestSupport.withTemporaryDirectory { directory in
            ...
            let executable = try TestSupport.writeTestExecutable(
                #"""
                printf '%s' '{"protocol_version":4,"event":"started",'
                sleep 0.2
                printf '%s' '"command":"convert","run_id":"r1"}'
                sleep 0.2
                printf '%s' '
                {"protocol_version":4,"event":"finis'
                sleep 0.2
    ...
    func finalLineWithoutNewlineIsDelivered() async throws {
        try await TestSupport.withTemporaryDirectory { directory in
            let executable = try TestSupport.writeTestExecutable(
                #"""
                printf '%s' '{"protocol_version":4,"event":"finished","status":"complete","exit_status":0}'
                exit 0
                """#,
                in: directory
            )
