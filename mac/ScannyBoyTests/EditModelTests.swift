...
struct EditModelTests {
    ...
    private static func negativeJSON(
        ...
        return """
            ...
            "tone_snap_gamma":\(toneSnapJSON),\
            "tone_density":\(toneDensityJSON),"tone_shadow_density":null,\
            "tone_highlight_density":null}
            """
    }
    ...
    private static func toneParamsJSON(from adjustment: ToneAdjustment?) -> String {
        guard let adjustment else {
            return """
            "snap_gamma":null,"density":null,"shadow_density":null,\
            "highlight_density":null
            """
        }
        return """
            "snap_gamma":\(adjustment.snapGamma),\
            "density":\(adjustment.density),"shadow_density":\(adjustment.shadowDensity),\
            "highlight_density":\(adjustment.highlightDensity)
            """
    }
    ...
    private static func editToneRunner(
        ...
        let toned = rollInfoEvent(negatives: negativeIDs.enumerated().map { index, id in
            negativeJSON(
                negativeID: id, sequence: index + 1, intended: nil, applied: nil,
                toneSnapGamma: adjustment?.snapGamma,
                toneDensity: adjustment?.density
            )
        })
    ...
    func testSetToneAppliesToTheWholeSelection() async throws {
        ...
        let runner = try Self.editToneRunner(
            ...
            adjustment: ToneAdjustment(
                snapGamma: 0.2, density: 1, shadowDensity: 0, highlightDensity: 0
            )
        )
        let model = try await Self.multiSelectModel(runner)
    ...
    func testRenderGenerationCarriesTone() {
        ...
        let other = Self.multiNegative(id: "n1", toneSnapGamma: 0.5)
    ...
    func testRenderGenerationCarriesColor() {
        let neutral = Self.multiNegative(id: "n1", toneSnapGamma: nil)
        let separated = Self.multiNegative(
            id: "n1", toneSnapGamma: nil, colorDyeSeparation: 1.2
        )
    ...
    func testScheduleToneDebounces() async throws {
        ...
        model.scheduleTone(
            model.selectionTargets,
            adjustment: ToneAdjustment(
                snapGamma: 0, density: 1, shadowDensity: 0, highlightDensity: 0
            )
        )
        model.scheduleTone(
            model.selectionTargets,
            adjustment: ToneAdjustment(
                snapGamma: 0.5, density: 1, shadowDensity: 0, highlightDensity: 0
            )
        )
        ...
        #expect(count == 1)
        #expect(model.visibleNegatives[0].toneSnapGamma == 0.5)
    }
    ...
    /// Parses `--snap` from the fake CLI, increments `counter` on each
    /// `edit tone`, and emits matching `edit_recorded` events.
    ...
    private static func spotsRunner(
        ...
        let reviewed = Self.rollInfoEvent(negatives: [
            Self.negativeJSON(negativeID: "n1", sequence: 1, intended: nil, applied: nil)
                .replacingOccurrences(of: "\"tone_highlight_density\":null}", with: """
                    "tone_highlight_density":null,"spots":\(initialSpots)}
                    """)
        ])
    ...
    func testRenderGenerationCarriesSpots() {
        func negative(_ summary: NegativeSpots.Summary?) -> RollManifest.Negative {
            Self.multiNegative(id: "n1", toneSnapGamma: nil)
    ...
    func testStaleSummaryAffectsGeneration() {
        let fresh = Self.multiNegative(id: "n1", toneSnapGamma: nil)
        let stale = Self.multiNegative(id: "n1", toneSnapGamma: nil)
            .with(
        ...
        let cropped = Self.rollInfoEvent(negatives: [
            Self.negativeJSON(negativeID: "n1", sequence: 1, intended: nil, applied: nil)
                .replacingOccurrences(
                    ...
                    with: "\"tone_highlight_density\":null,\"crop\":\(cropReport)}"
                )
        ])
    ...
    func testRenderGenerationCarriesCrop() {
        func negative(_ crop: CropState?) -> RollManifest.Negative {
            var base = Self.multiNegative(id: "n1", toneSnapGamma: nil)
            base.crop = crop
            return base
        }
        let none = negative(nil)
