...
def test_solve_nulls_the_recorded_residual():
    ...
    display offsets whose R-G and B-G deviations approximately negate the
    recorded residual.  The projection onto 2 axes is lossy, so the match
    is approximate (within ~10 %)."""
    ...
    # The display offsets' R-G and B-G should approximately negate the
    # residual — the 2-axis projection can't perfectly cancel a 3-channel
    # residual, but the sign and rough magnitude must match.
    rg = display[0] - display[1]
    bg = display[2] - display[1]
    assert rg == pytest.approx(-0.06, abs=0.02)
    assert bg == pytest.approx(0.03, abs=0.02)
    assert color._luma_weighted_sum(offsets) == pytest.approx(0.0, abs=1e-12)


def test_the_residual_to_display_offsets_is_luma_neutral():
    for a, b in ([0.06, -0.03], [0.0, 0.0], [-0.5, 0.25]):
        o = auto_color._residual_to_display_offsets(a, b)
...
def test_the_solve_balance_is_independent_of_color_params():
    """solve_balance is a pure projection from residual to balance axes;
    it does not depend on cast_removal or cast_removal_highlights."""
    ...
    same = auto_color.solve_balance(record, params, 1.55, 0.5)
