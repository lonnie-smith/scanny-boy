...
prediction, the ordinary (non-nested) IFD0 tags, and an embedded ICC
profile.

A later pass rewrites this file with `tifftools` to add the nested EXIF
...
Writing the TIFF with `tifffile`: four rules are not obvious and are each
followed exactly here — `metadata=None` (avoids a duplicate
`ImageDescription`), `description=`/`software=` keyword arguments
(`extratags` silently drops both), `iccprofile=` (not `extratags`), and
Adobe Deflate's TIFF compression code `32946` (not `8`).
"""
...
# reach this writer. Never copy the source Orientation value.
OUTPUT_ORIENTATION = 1


def software_tag_value() -> str:
    """The `Software` tag value: "Scanny Boy <version>"."""
