...
# Protocol 18 adds the film-extent pass (docs/BLACK_POINT_REFINEMENT.md):
# the per-negative `normalization` block gains a `film_extent` finding, and
# the stitch stage emits `NORMALIZE_FILM_EXTENT_WITHHELD` (informational:
# a non-film border band was withheld from the meters, insets named in
# canvas pixels) and `NORMALIZE_FILM_EXTENT_EXCESSIVE` (warning: the
# withheld rect kept less than half the analysis region — the frame is
# unusual, and the user should look at it). Both ride the warning event
# channel; severity is recorded in CONTRACT.md's code table.
PROTOCOL_VERSION = 18
