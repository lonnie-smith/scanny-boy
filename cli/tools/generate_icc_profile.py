...
encoding, so the wide container is the honest choice. Three profiles:
...
A second construction path builds the **export** profiles with lcms2: the
carry-over trick cannot produce
...
- `ScannyBoy-Export-AdobeRGB-v1.icc` — the export's RGB space, whose
  ...
  this space (the export's matrix conversion).
...
def generate_profile(
    ...
    *,
    ...
    The keyword arguments exist for the grey profile: `trc_signatures`
    replaces the source's rTRC/gTRC/bTRC
...
def generate_density_grey_profile() -> bytes:
    """The gray-class companion of the density profile: same gamma-2.2
    viewing TRC, one kTRC instead of the RGB
...
# The required disclaimer lives in `cprt`, not `desc`: it is a copyright
# notice, `cprt` is the tag for one, and a `desc` this long is precisely
# what macOS refuses to display.
...
# shared with `icc_profile.TRC_G_EXPORT` and `render.GAMMA_ADOBE`. lcms2
# writes it into a `para` type-0 curve, the same shape the other three
...
def generate_export_rgb_profile() -> bytes:
    """The export's RGB space: Adobe RGB (1998)'s
...
def generate_export_grey_profile() -> bytes:
    """The grey-class export companion: the same TRC as the RGB profile —
    a mono roll and a colour roll of the same scene must have identical
    tonality — with a D50 `wtpt` and the same `chad`, and no colorants
    (GRAY class).

    lcms2's gray profile writes a D65 `wtpt` and no `chad`, which is the
    older non-conformant convention; the conformant D50 + `chad` pair is
    set here."""
    profile = cms_profile(
        "gray",
        whitepoint=ADOBE_RGB_WHITEPOINT_XY,
